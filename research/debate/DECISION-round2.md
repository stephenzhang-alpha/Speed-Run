...
**Status:** authoritative for the *second* wave of features. It builds on
...
- **Problem it fixes:** the current Choke Index is `all-timed-accuracy −
  all-untimed-accuracy` over *different* item pools → bakes in an item-difficulty
...
and it attacks the #1 real failure mode no shipped feature touches — *not showing
up*.
- **Mechanism:** one if-then sentence at onboarding ("If it's `<time>` after
  ...
  loss-aversion**; after ≥2 missed planned days a *neutral* re-plan prompt (never
...
- **Eval:** `eval/adherence.py` — 14-day planned-completion rate plan vs no-plan
  (CI excludes 0); **guardrail:** accuracy-per-item CI must *include* 0
...
- **Eval:** `eval/feedback.py` — 3-arm equal-*time* (KR / KCR+label / EF-contrast);
...
cumulative time-on-task (F4 measures per-item *time pressure*, orthogonal).
...
cross-cutting LR primitive, today only *tagged*, never *drilled*. 7.3,
...
*Full per-proposal research memos, evidence ratings, and round-by-round scores are
