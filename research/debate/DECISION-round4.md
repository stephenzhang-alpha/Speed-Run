...
Same honesty discipline: a learning _claim_ ships only behind a measured `eval/`
...
_careless_ error (reverting to the prepotent task on an inverted stem).
Automatization makes the correct attentional set resource-independent, so it is
_more_ robust at the fatigue moment the lapse strikes.
...
- **#22 Conditional-Chain Trainer** — SHIPPED. Fully deterministic: structured
  ...
  (truth-table, complete) with reachability kept as a proven-_sound_ explanation
