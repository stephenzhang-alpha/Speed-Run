...
convenience with an acceptable error rate — it is a _net-negative_ that installs
...
pre-validated **gate** — that alone owns correctness. In one line: _the AI
proposes, the oracle disposes._
...
- Trusting raw LLM generation as correct, or using AI to _synthesize_ the answer
...
_false rule_, and it teaches it most efficiently to the strongest students, who
encode it with high confidence. Learning science makes the cost precise: high-
confidence errors are the hardest to remove; they persist and even _return_ after
...
does _logically inert_ work — proposing a step ordering the oracle must bless,
...
decision procedure that _enumerates and proves_ it. A hallucinated step cannot
...
assumption: here the model is the _least_ trusted component.
...
`build_worked_example()`. Evil twins invert the same split: the oracle _enumerates_
...
what it ships is _confidently wrong_. If generation is now a commodity, then the
scarce, hard-to-copy artifact is the **verification infrastructure** — and
specifically a checker you have _earned the right to trust_. Ask an LLM to build a
...
against a gold set of known-correct/known-wrong items _before_ it is trusted, with
...
[`../lsat/ai/checker.py`](../lsat/ai/checker.py) runs as a _separate_ call,
...
sits _below_ the LLM judge: the generator programmatically checks that each
...
flips with 0 mislabels cross-checked against an _independent_ oracle — are the
...
and — to a skeptical retaker hunting a 170 — _less_ persuasive than a product that
...
so**. Trust is the _enabling condition_, never the pitch. Two corollaries follow.
...
on any model bump" rule and which surfaces are model-_independent_. Grounded
...
`validate_hypotheses()` (pass rate ≥ 0.9) _before_ anything renders; the LLM may
only _rephrase_ a catalog entry, and the rephrase is fail-closed (label must
...
- **Expert 1 — Janet Metcalfe**
  ...
  - **Focus:** The **hypercorrection effect** — errors made with _high_ confidence
    are corrected best after feedback, yet also tend to _return_ over a delay unless
    re-tested.
  - **Why Follow:** Her work is the empirical backbone of SPOV 1. It is why a
    confidently-wrong AI output is _maximally_ damaging (it installs the hardest-to-
...
- **Expert 2 — Daniel T. Willingham**
  - **Who:** Professor of Psychology, University of Virginia; author of _Why Don't
  ...
  - **Why Follow:** Justifies treating LSAT reasoning as a _domain_ with a fixed,
...
- **Expert 3 — John Sweller**
  ...
  - **Why Follow:** Grounds the _faded_ worked-example design and explains why a
    _correct_ worked example is high-leverage — and, by the same token, why a wrong
    one is uniquely costly.
  - **Where:** Sweller (1988), _Cognitive Science_ —
    <https://doi.org/10.1207/s15516709cog1202_4>

- **Expert 4 — Robert A. Bjork & Elizabeth L. Bjork**
  - **Who:** Distinguished Professors of Psychology, UCLA (Bjork Learning and
    Forgetting Lab).
  - **Focus:** **Desirable difficulties**, retrieval practice, and the _fluency
    illusion_ (conditions that feel easy often teach worse).
  - **Why Follow:** The "harder-but-honest" stance — make the learner discriminate,
    ...
    also warns that learners will _mis-rate_ the most effective drills.
...
- **Expert 5 — Henry L. Roediger III & Jeffrey D. Karpicke**
  ...
  - **Focus:** The **testing effect** / retrieval-based learning — retrieval
    strengthens memory, and _feedback_ amplifies it.
  - **Why Follow:** The entire graded-retrieval loop the AI feeds sits on this base;
    it also frames _feedback quality_ (correct, trap-level) as a first-class design
...
- **Expert 6 — Edmund M. Clarke**
  ...
  - **Why Follow:** The intellectual license for "the oracle disposes." A bounded
    ...
    model checking is the discipline of trusting a _decision procedure_ over
...
- **Expert 7 — Hunter Lightman et al. (OpenAI)**
  - **Who:** Authors of _Let's Verify Step by Step_ (2023).
  - **Focus:** **Process supervision** — verifying/rewarding each _reasoning step_,
...
- **Expert 8 — Lianmin Zheng et al.**
  - **Who:** Authors of _Judging LLM-as-a-Judge with MT-Bench and Chatbot Arena_
    (2023).
  ...
  - **Why Follow:** Names both the promise and the peril of the LLM-judge pattern,
    which is precisely why our checker is _independent_ **and** _validated on a gold
    set for false-pass_ before it is trusted as a gate (SPOV 2).
...
- **Expert 10 — David M. Killoran / PowerScore**
  - **Who:** Author of the PowerScore _LSAT Logical Reasoning Bible_.
...
- **Insight 1 — The verifier–generator asymmetry becomes a guarantee only where the
  logic is decidable.** In complexity terms, _checking_ a candidate solution is
  routinely cheaper and more reliable than _producing_ one. The LSAT's formal-logic
  ...
  checker can be made _total and provably sound_ while the generator stays fallible.
  ...
  guarantee — and it is _why_ the feature invests engineering in `entails()` /
  `classify()` rather than in a better prompt.

- **Insight 2 — A wrong AI card manufactures the single worst kind of error.** The
  product's own Thesis 2 says the misses worth deep review are the _confident_ ones,
  ...
  example does not just waste a review, it _installs_ a high-confidence misconception
  in exactly the students most able to encode it firmly. So the learning science that
  motivates the confidence engine independently _forbids_ trusting AI output —
...
- **Insight 3 — Erroneous-example pedagogy is only safe when "wrong" is provably
  ...
  pairing a correct case with a _wrong_ one and asking the learner to find the
  difference produces strong transfer — but _only_ if the wrong case is wrong on
  ...
  flips, so the "instructive wrong answer" is _guaranteed_ wrong-on-one-axis. The
...
- **Insight 4 — Once content is free, the moat moves from the prompt to the
  ...
  cannot cheaply copy a checker whose false-pass rate has been _measured to zero_ on
...
- **Insight 5 — Abstention is a trust technology, not a limitation.** Because
  calibration-training has a weak empirical record (you can _measure_ miscalibration
  but not reliably coach it away) and far-transfer claims are easily overstated, the
  honest move elsewhere in the product is to _widen ranges and abstain_ rather than
  ...
  correctness claim) is _more_ convincing to a skeptical retaker than a blanket
  guarantee would be.
...
- **Insight 6 — A gated guarantee is only as stable as the pinned model, so the
  ...
  bureaucratic caution; it is what keeps the eval numbers _true_ over time. Its
...
- **Insight 7 — "Untrusted by default" is one principle applied at two layers.** The
  pipeline distrusts _both_ the model's output and the model's input: the checker
  ...
  never obeyed. This is a single stance — _guilty until proven_ — applied to output
  and to input. It is also why the rule-based `OfflineClient` is a _feature_ rather
...
- **Source: `lsat/worked_example.py`**
  ...
  - **DOK 2 — Summary:** The LLM may only _order_ moves; a material-entailment oracle
...
- **Source: `lsat/evil_twin.py`**
  - **DOK 1 — Facts:**
    ...
    - `_ai_select()` lets the LLM choose only a twin _index_ and fresh _nouns_; an
...
- **Source: `lsat/ai/generator.py`**
  ...
  - **DOK 2 — Summary:** Defense-in-depth _below_ the LLM judge; catches fabricated
...
- **Source: `lsat/ai/client.py`**
  - **DOK 1 — Facts:** `LLMClient` is a `Protocol`; `OfflineClient` is a purely rule-
    based stand-in that _cannot_ follow embedded instructions; `ClaudeClient` raises
...
- **Source: `lsat/ai/misconception.py`**
  - **DOK 1 — Facts:** `HYPOTHESIS_CATALOG` maps documented taxonomy node ids →
    ...
    = 0.9) _before_ anything renders; `rephrase_with_ai()` is fail-closed (label
    survives, no new content/numbers) and degrades silently.
  - **DOK 2 — Summary:** The "grounded-gold" tier — the AI may only _rephrase_ a
...
_(Compiled with DOIs in [`Speedrun_Research_Support.md`](Speedrun_Research_Support.md)
and the memos in [`research/`](research/R2-error-trap-analysis.md).)_

#### B.1 Errors and confidence (hypercorrection)

- **Source: Metcalfe (2017), "Learning from Errors," _Annual Review of Psychology_ 68.**
  - **DOK 1 — Facts:** Errorful retrieval + corrective feedback out-learns error-free
    study; _analysis of the reasoning behind the error_ is crucial; corrected high-
...
- **Source: Butterfield & Metcalfe (2001), _JEP:LMC_ 27(6).**
  - **DOK 1 — Facts:** The hypercorrection effect — errors committed with _high_
...
- **Source: Butler, Fazio & Marsh (2011); Metcalfe & Miele (2014).**
  - **DOK 1 — Facts:** High-confidence errors _return_ over a week; a retrieval test
...
- **Source: Große & Renkl (2007), _Learning and Instruction_ 17(6); Booth et al. (2013).**
  - **DOK 1 — Facts:** Mixing correct + _incorrect_ worked examples improved transfer
...
- **Source: Schwartz & Bransford (1998); Loewenstein, Thompson & Gentner (1999).**
  - **DOK 1 — Facts:** Comparing two minimally-different cases _before_ being told the
...
- **Source: Sweller (1988); Sweller, van Merriënboer & Paas (1998).**
  ...
  - **DOK 2 — Summary:** Justifies the faded worked-example format and raises the cost
    of a _wrong_ worked step (it consumes scarce load teaching a falsehood).
...
- **Source: Willingham (2007, 2009).**
  ...
  - **Link to source:** _American Educator_ (2007) —
...
- **Source: Clarke, Grumberg & Peled, _Model Checking_ (MIT Press, 1999); ACM Turing
  Award 2007.**
  - **DOK 1 — Facts:** Model checking exhaustively verifies whether a (finite-state)
    system satisfies a formal specification; it is a _decision procedure_, not a
    heuristic.
...
- **Source: Lightman et al. (2023), "Let's Verify Step by Step" (OpenAI).**
  - **DOK 1 — Facts:** Process supervision (rewarding each correct _step_) outperforms
  ...
  - **DOK 2 — Summary:** Published analogue of per-step oracle verification of a
    derivation; supports checking the _steps_, not just the conclusion.
...
- **Source: Zheng et al. (2023), "Judging LLM-as-a-Judge with MT-Bench and Chatbot
  ...
  - **DOK 2 — Summary:** Why the checker is _independent and gold-validated_ — an
...
- **Source: 7Sage / J.Y. Ping, "The Blind Review."**
  - **DOK 1 — Facts:** Flag uncertain questions during a timed run, re-answer untimed
    before checking, and study hardest the ones you were _confident about yet missed_
...
- **Source: PowerScore / Killoran (LR taxonomy + wrong-answer families); Sadler
  ...
  - **DOK 1 — Facts:** LR items sort into a fixed question-type taxonomy with a
    prescribed attack; wrong answers are _engineered_ along known axes; a chosen
