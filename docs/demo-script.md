...
- **Do:** flip **Tools → LSAT Study Mode** to *Blind Review*, re-answer an item
  untimed; open the dashboard's insight grid.
- **Point at:** the **Paired Choke Index** CI number-line (a *confident* pacing
...
- **Say:** "We capture *how* you got it wrong — the distractor you picked, your
...
- **Do:** on the phone, open the **Logic** tab → **Validity**: read a
  quantifier argument and tap *must / cannot / could*; then **Negation**: pick the
  exact negation (show that ¬"most" is "at most half," not "most … not").
- **Point at:** back on the dashboard, the **"Your progress"** section — a
  per-skill *self-referential* up/down delta that only appears when its CI clears
  zero (never a rank or a fake readiness number) — and the **Rush-Error** and
  **Time-Leak** tiles.
- **Say:** "The quantifier drill's answer key is *proven*, not hand-typed — a Venn
  ...
  are diagnostics measured against your *own* baseline. All four are round-3
...
- **Do:** from the Study tab tap **Start a timed section** → answer a few items
  under the LSAT-pace clock, *change* one answer, submit; show the **First-Instinct
  Ledger** result.
- **Point at:** the ledger's *your own* wrong→right vs right→wrong tally with a CI
...
- **Say:** "The section runner never reveals correctness mid-section — it sends
  your raw choices and the *server* grades — so the ledger tells you whether
  changing answers helps *you*, refusing the folk 'never change' rule and the
