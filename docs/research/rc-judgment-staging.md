...
1. **Label correctness needs a human.** Whether a main-point candidate is truly
   ...
   **and** a human spot-check. I can build the deterministic *grader* (verifiable),
...
- `lsat/rc_judgment.py`: the deterministic grader (pick vs labels; optional
  ...
  buildable and unit-testable now; it is the *content* that is the blocker.
...
four other round-3 winners (quantifier, growth, rush, time-leak) *were* shipped
